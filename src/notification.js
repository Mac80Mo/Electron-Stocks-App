...
// Nur im Hauptfenster ausführen (DOM-Elemente nur dort vorhanden)
if (document.getElementById('noticount')) {
    dbcount()
}
if (document.getElementById('bulmaNoti')) {
    querryNoti()
}
// Im Notification-Fenster die Liste laden
        ...
        if (succ) {
            for (let i = 0; i < data.length; i++) {
                ...
                if (price >= maxvalue || price <= minvalue) {
                    let x = document.getElementById('bulmaNoti')
                    x.style.display = "block"
                    document.getElementById('bulmaNoti').innerHTML = '<button class="delete" onclick="hideBulmaNoti()"></button> <p>Preisziel erreicht!</p> <button class="button" onclick="notify(`Ǹotification`)">Notification</button>'
